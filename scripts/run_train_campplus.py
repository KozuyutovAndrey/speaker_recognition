#!/usr/bin/env python3
"""Fine-tune CAM++ (Alibaba DAMO) with SubcenterArcFace on competition data.

Uses MUSAN noise + RIR augmentations. CAM++ pretrained on VoxCeleb is a strong
starting point — fine-tuning adapts it to our speakers and distortion profile.

Usage:
    bash scripts/run.sh scripts/run_train_campplus.py \
        --config configs/train_campplus.yaml
"""
...
def build_augmentation(cfg) -> AugmentationPipeline | None:
    ...
    if aug_cfg.noise_dir:
        ...
        print(f"[run_train_campplus] Noise files: {len(noise_files)}")

    if aug_cfg.rir_dir:
        rir_dir = Path(aug_cfg.rir_dir)
        rir_files = list(rir_dir.rglob("*.wav")) + list(rir_dir.rglob("*.flac"))
        print(f"[run_train_campplus] RIR files: {len(rir_files)}")
...
def main():
    ...
    print(f"[run_train_campplus] Device: {device}")
    ...
    print(f"[run_train_campplus] Train speakers: {n_train_speakers}, "
          f"train files: {len(train_df)}, val files: {len(val_df)}")

    print("[run_train_campplus] Loading CAM++ pretrained model...")
    encoder = CAMPlusWrapper(
        freeze_frontend=cfg.model.freeze_frontend,
        emb_dim=cfg.model.emb_dim,
    ).to(device)
    n_params = sum(p.numel() for p in encoder.parameters()) / 1e6
    print(f"[run_train_campplus] CAM++ params: {n_params:.1f}M")
    ...
    print(f"[run_train_campplus] Augmentation: {'enabled' if augment else 'DISABLED'}")

    save_name = cfg.checkpoint.get("save_name", "campplus_finetune_best.pt")
    ...
    if args.resume:
        ...
        print(f"[run_train_campplus] Resumed from: {args.resume_from}")

    if args.reset_best:
        trainer.best_p10 = 0.0
        print("[run_train_campplus] best_p10 reset to 0.0")

    trainer.fit()

    print(f"\n[run_train_campplus] Best checkpoint: {cfg.checkpoint.dir}{save_name}")
