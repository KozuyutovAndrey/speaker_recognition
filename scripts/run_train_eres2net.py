#!/usr/bin/env python3
"""Fine-tune ERes2Net (Alibaba DAMO) with SubcenterArcFace on competition data.

Uses the same recipe as successful CAM++ fine-tune:
- GPU-batched torchaudio MelSpectrogram frontend (bypasses sequential Kaldi.fbank)
- MUSAN noise + RIR augmentations
- SubcenterArcFace loss

Usage:
    bash scripts/run.sh scripts/run_train_eres2net.py \
        --config configs/train_eres2net.yaml
"""
...
def build_augmentation(cfg) -> AugmentationPipeline | None:
    ...
    if aug_cfg.noise_dir:
        ...
        print(f"[run_train_eres2net] Noise files: {len(noise_files)}")

    if aug_cfg.rir_dir:
        rir_dir = Path(aug_cfg.rir_dir)
        rir_files = list(rir_dir.rglob("*.wav")) + list(rir_dir.rglob("*.flac"))
        print(f"[run_train_eres2net] RIR files: {len(rir_files)}")
...
def main():
    ...
    print(f"[run_train_eres2net] Device: {device}")
    ...
    print(f"[run_train_eres2net] Train speakers: {n_train_speakers}, "
          f"train files: {len(train_df)}, val files: {len(val_df)}")

    print("[run_train_eres2net] Loading ERes2Net pretrained model...")
    encoder = ERes2NetWrapper(
        freeze_frontend=cfg.model.freeze_frontend,
        emb_dim=cfg.model.emb_dim,
    ).to(device)
    n_params = sum(p.numel() for p in encoder.parameters()) / 1e6
    print(f"[run_train_eres2net] ERes2Net params: {n_params:.1f}M")
    ...
    print(f"[run_train_eres2net] Augmentation: {'enabled' if augment else 'DISABLED'}")

    save_name = cfg.checkpoint.get("save_name", "eres2net_finetune_best.pt")
    ...
    print(f"\n[run_train_eres2net] Best checkpoint: {cfg.checkpoint.dir}{save_name}")
